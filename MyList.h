...
public:
	...
	void addListBack(MyList<T>& other);
	Node<T>& begin();
	Node<T>& end();
...
inline MyList<T>::MyList(const MyList<T>& other)
{
	...
	while (iter)
	{
		Node<T>* copyElment = new Node<T>(iter->getElement(), nullptr, nullptr);
...
inline void MyList<T>::add(const T& element)
{
	...
	Node<T>* newNode = new Node<T>(copyElement, nullptr, tail);
...
inline void MyList<T>::addFront(const T& element)
{
	++size;
	T copyElement = element;
	Node<T>* newNode = new Node<T>(copyElement, head, nullptr);
	head = newNode;
}

template<typename T>
inline void MyList<T>::addListBack(MyList<T>& other)
{
	for (T k : other) { add(k); }
	add(tail->getElement());
}
